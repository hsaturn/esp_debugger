...
MainWindow::MainWindow() : QMainWindow()
{
    ...
    setWindowTitle(tr("Esp Debugger"));
...
{
   QMessageBox::about(this, tr("About Esp_debug"), tr("Esp_debug, graphical debugger for ESP8266 (c) hsaturn 2022"));
}
...
*/
    QAction *preferencesAct = addMenuEntry("&File", "&Preferences");
    connect(preferencesAct, &QAction::triggered, this, &MainWindow::setPreferences);
...
void MainWindow::setPreferences()
{
    ...
    QString gdb = Settings.value("prefs/elf-gdb").toString();
    std::cout << "gdb=" << gdb.toStdString() << std::endl;
    QFileDialog fileDialog(this);
    fileDialog.setFilter(QDir::AllEntries | QDir::Hidden);
    gdb = fileDialog.getOpenFileName(this, "Select xtensa-lx106-elf-gdb binary", gdb);
    if (gdb.length()) Settings.setValue("prefs/elf-gdb", gdb);
}
